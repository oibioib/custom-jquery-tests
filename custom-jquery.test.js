  ...
  describe('append', () => {
    ...
    it('Should insert string to the end of each element', () => {
      ...
      document.querySelectorAll('div').forEach((el) => {
        expect(el.innerHTML.includes(testString, -testString.length)).toBe(
          true
        );
      });
    });
    ...
    it('Should insert Array to the end of each element', () => {
      ...
      document.querySelectorAll('div').forEach((el) => {
        expect(
          el.innerHTML.includes(testArray.join(''), -testArray.length)
        ).toBe(true);
      });
    });
  ...
  describe('html', () => {
    ...
    beforeEach(() => {
      document.body.innerHTML =
        '<div><span>Test div1</span></div><div><span>Test div2</span></div>';
      $div = $('div');
    });
